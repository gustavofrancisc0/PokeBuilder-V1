...
function populateTypeSelects() {
    const options = state.types.map(type => {
        const name = typeNames[type.name] || type.name;
        return `<option value="${type.name}">${name}</option>`;
    }).join('');
    
    elements.type1.innerHTML = '<option value="">Selecione um tipo...</option>' + options;
    ...
    card.innerHTML = `
        ...
        <div class="pokemon-card-body">
            ...
            <div class="pokemon-types">
                ${pokemon.types.map(t => `
                    <span class="pokemon-type bg-${t.type.name}">${typeNames[t.type.name] || t.type.name}</span>
                `).join('')}
            </div>
            <div class="pokemon-matchups">
                <div class="matchup-row">
                    ...
                    <div class="matchup-types">
                        ${matchups.strengths.slice(0, 4).map(t => `
                            <span class="matchup-type bg-${t}">${typeNames[t] || t}</span>
                        `).join('') || '<span style="color: #999; font-size: 0.8rem;">-</span>'}
                    </div>
                </div>
                <div class="matchup-row">
                    <span class="matchup-label">Fraco vs:</span>
                    <div class="matchup-types">
                        ${matchups.weaknesses.slice(0, 4).map(t => `
                            <span class="matchup-type bg-${t}">${typeNames[t] || t}</span>
                        `).join('') || '<span style="color: #999; font-size: 0.8rem;">-</span>'}
                    </div>
                </div>
...
function updateTeamAnalysis() {
    ...
    // Renderizar
    elements.teamWeaknesses.innerHTML = sortedWeaknesses.length > 0 
    ...
    elements.teamResistances.innerHTML = sortedResistances.length > 0
    ...
    elements.teamImmunities.innerHTML = immunitySet.size > 0
...
function openPokemonModal(pokemon) {
    ...
    elements.modalBody.innerHTML = `
        <div class="modal-pokemon-header" style="--type-color: ${typeColors[primaryType]}">
        ...
        <div class="modal-pokemon-body">
            ...
            <p class="modal-pokemon-number">#${String(pokemon.id).padStart(3, '0')}</p>
            
            <div class="modal-section">
                <h4>Tipos</h4>
                <div class="modal-types">
                    ${pokemon.types.map(t => `
                        <span class="modal-type bg-${t.type.name}">${typeNames[t.type.name] || t.type.name}</span>
                    `).join('')}
                </div>
            </div>
                ...
                <div class="modal-matchups-grid">
                    <div class="modal-matchup-card">
                        ...
                        <div class="type-badges">
                            ${matchups.weaknesses.map(t => `
                                <span class="type-badge bg-${t}">${typeNames[t] || t}</span>
                            `).join('') || '<span style="color: #999;">Nenhum</span>'}
                        </div>
                    </div>
                    <div class="modal-matchup-card">
                        <h5>🛡️ Resiste a</h5>
            ...
            <button class="modal-btn-add" 
                    id="modalAddBtn"
                    ${isInTeam || state.team.length >= 6 ? 'disabled' : ''}>
                ${isInTeam ? '✓ Já está no Time' : state.team.length >= 6 ? 'Time Completo' : '+ Adicionar ao Time'}
            </button>
        </div>
    `;
...
async function loadAllMoves(pokemon, filter = 'all') {
    ...
    let filteredMoves = [];
    if (filter === 'all') {
        filteredMoves = [...movesByMethod['level-up'], ...movesByMethod['machine'], ...movesByMethod['tutor'], ...movesByMethod['other']];
    } else {
    ...
    if (moveDetails.length === 0) {
        movesList.innerHTML = '<p style="text-align: center; color: #999; padding: 20px;">Nenhum movimento encontrado.</p>';
        return;
    }
    
    movesList.innerHTML = moveDetails.map((move, index) => {
        if (!move) return '';
        ...
        return `
            <div class="move-card ${isSelected ? 'selected' : ''}" data-move-name="${move.name}">
                <div class="move-header">
                    <div class="move-info">
                        <span class="move-name">${formatMoveName(move.name)}</span>
                        <span class="move-type bg-${move.type.name}">${typeNames[move.type.name] || move.type.name}</span>
                        <span class="move-class ${damageClass}">${formatDamageClass(damageClass)}</span>
                    </div>
                ...
                <div class="move-stats">
                    ${moveInfo.method === 'level-up' ? `<span class="move-method">Nv. ${moveInfo.level}</span>` : ''}
                    ${moveInfo.method === 'machine' ? `<span class="move-method">TM/HM</span>` : ''}
                    ${moveInfo.method === 'tutor' ? `<span class="move-method">Tutor</span>` : ''}
    ...
    // Eventos de seleção
    movesList.querySelectorAll('.move-select-btn').forEach(btn => {
...
function toggleMoveSelection(pokemonId, moveData) {
    ...
    const moveCard = document.querySelector(`.move-card[data-move-name="${moveData.name}"]`);
    if (moveCard) {
    ...
    if (selectedMoves.length >= 4) {
        document.querySelectorAll('.move-select-btn:not([disabled])').forEach(btn => {
            const moveData = JSON.parse(btn.dataset.move);
            const isSelected = selectedMoves.some(m => m.name === moveData.name);
            if (!isSelected) {
                btn.disabled = true;
...
function renderSelectedMoves(pokemonId) {
    ...
    if (selectedMoves.length === 0) {
        selectedMovesDiv.innerHTML = '<p class="no-moves-selected">Nenhum movimento selecionado (0/4)</p>';
        return;
    }
            ...
            ${selectedMoves.map(move => `
                <div class="selected-move-card">
                    <div class="selected-move-header">
                        <span class="selected-move-name">${move.displayName}</span>
                        <button class="remove-move-btn" data-move-name="${move.name}">×</button>
                    </div>
                    <div class="selected-move-info">
                        <span class="move-type bg-${move.type}">${typeNames[move.type] || move.type}</span>
                        <span class="move-class ${move.damageClass}">${formatDamageClass(move.damageClass)}</span>
                    </div>
